...
async def translate_article(article_url: str) -> Dict[str, Any]:
    """
    Manually trigger translation for a specific article.
    新流程：從資料庫提取原始資料 → OpenAI翻譯 → 更新回資料庫
    ...
    try:
        ...
        from .db import fetch_articles, update_article_translation
        ...
        # 更新繁體中文翻譯
        if translation_result.get('title_zh_tw'):
            total_updates += 1
        ...
        # 更新簡體中文翻譯
        if translation_result.get('title_zh_cn'):
            total_updates += 1
            success = await update_article_translation(
                article_link=article_url,
        ...
        # 更新英文翻譯
        if translation_result.get('title_en'):
            total_updates += 1
            success = await update_article_translation(
                article_link=article_url,
                target_language="en",
        ...
        if success_count == 0:
            raise HTTPException(status_code=500, detail="Failed to update any translations")
        ...
        return {
            ...
            "total_translations": total_updates
        }
